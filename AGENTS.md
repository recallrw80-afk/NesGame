...
1. **先修 REL/IND 的 3 个逻辑 bug**（见上 4/5）——让用户自己改，再一起编译确认
2. **实现全部指令函数体 + reset/step/nmi/irq**（大工程，教学模式推进）——链接目前失败于此，是必经之路
3. 实现 APU 和 DMA
...
- **⚠️ 重要：用户主动提出"写了大量代码但仍无法真正理解"的瓶颈。必须换讲法——** 不能只给"照抄 x 换 y"式模板，要讲透底层心智模型。核心：一个寻址模式的本质职责是回答**同一个问题**——"如何根据指令字节 + 寄存器算出 operand 真正所在的内存地址？"。讲的时候统一成一个框架（**所有模式都在填 `addr_abs`**），并回到 CPU 的"取指-译码-执行"大循环解释每个内部变量（opcode/pc/fetched/addr_abs/addr_rel）的意义。放慢节奏，确认每一步理解后再前进，不要堆代码。
